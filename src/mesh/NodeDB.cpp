...
static void installTrafficManagementDefaults(meshtastic_LocalModuleConfig &mc)
{
    ...
    // Fork aggressive defaults. The redesigned proto uses "non-zero implies enabled"
    ...
    mc.traffic_management.rate_limit_max_packets = 30;           // max packets per node per window
    mc.traffic_management.unknown_packet_threshold = 5;          // drop source after N unknown pkts/window
#endif
}
